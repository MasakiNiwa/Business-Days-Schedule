...
const TASKS: Task[] = [
  ...
  {
    group: 'share',
    goal: '来月の予定だけを一覧で見たい・書き出したい',
    answer:
      'カレンダーで見たい月へ移ってから、〈一覧〉に切り替えるか〈書き出し〉を開いてください。' +
      'その月が引き継がれます。一覧の起点は上の日付欄で選び直せます。' +
      '今日からに戻したいときは〈今日から〉を押してください。',
    section: 'marks',
  },
  {
    group: 'share',
...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    rows: [
      ...
      {
        term: 'その月に営業日が足りないとき',
        body: '「第25営業日」のように、月によっては存在しない指定もできます。作れない回があると、保存ボタンのそばに「『確定処理』は 10 件の回で日付を決められません」と件数が出ます。「内訳を見る」を押すと、その回ごとの説明（「2026年10月には月初から25営業日目がありません」）へ移動します。黙って消えることはありません。',
      },
      {
        term: '入力を間違えたとき',
        body: 'おかしな値はその場で直したりせず、そのまま残してエラーを出します。画面に出ている値と保存される値は必ず一致します。エラーはその予定の欄の直下に出て、保存しようとするとその欄へ移動します。',
      },
    ],
    notes: [
  ...
  },
  {
    ...
    title: 'カレンダーの見かた',
    lead:
      'カレンダーと一覧は、見ている期間を引き継ぎます。' +
      '別の月を見てから一覧や書き出しへ移ると、その月が最初から選ばれています。',
    rows: [
      { term: '←10', body: '本来10日の予定が、10日が休業日のため前の営業日へ移動してきたもの。' },
      { term: '→10', body: '本来10日の予定が、10日が休業日のため次の営業日へ移動してきたもの。' },
      { term: '破線の枠', body: '準備日・フォロー。本体の予定に紐づいて出ます。名前は「給与振込: 着金確認」のように、本体と前後の予定を並べて出します（カレンダー・一覧・日付の詳細で同じ）。' },
