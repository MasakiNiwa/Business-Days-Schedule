...
**M3 完了** — 月カレンダー表示、ルールの追加・編集・削除、営業日カレンダー設定、
JSON 入出力。残るは一覧（リスト）表示と印刷 CSS の仕上げです。

| | |
|---|---|
| ロジック層 | `src/core/` — 営業日判定・反復ルール展開・営業日補正・検証・入出力 |
| 表示層 | `src/ui/` — 月カレンダー、ルール一覧、編集フォーム、設定 |
| テスト | 205 件（`npm test`）。§6 の実務ユースケース16件を回帰テスト化 |
...
1. **ルールを追加** — 「毎月N日 / 第N営業日 / 第N曜日 / 毎週」から選び、
   休業日にあたったときの補正（前倒し・後ろ倒し・近い方・補正なし）を決めます。
   フォーム下部の**「次の10回」プレビュー**で、実際にどの日になるかをその場で確認できます。
2. **営業日を定義** — 設定から、週末の曜日・祝日を休むか・年末年始などの休業期間・
   臨時休業日・祝日出勤日を指定します。「自社」と「銀行」を別々に持てるので、
   社内の締めと振込を使い分けられます。
3. **バックアップ** — 設定の「エクスポート」で JSON に書き出せます。
