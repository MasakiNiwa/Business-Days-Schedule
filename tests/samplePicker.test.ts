...
describe('renderSamplePicker', () => {
  it('束の名前・説明・件数を出す', () => {
    const element = renderSamplePicker(packs, new Set(), { onAdd: vi.fn(), onClose: vi.fn() });
  ...
  it('追加を呼べる', () => {
    const onAdd = vi.fn();
    const element = renderSamplePicker(packs, new Set(), { onAdd, onClose: vi.fn() });
  ...
  it('追加済みの束は再追加として示す', () => {
    const element = renderSamplePicker(packs, new Set(['tax']), { onAdd: vi.fn(), onClose: vi.fn() });
    const first = element.querySelectorAll('.sample-item')[0];
    expect(first?.querySelector('.rule-badge')?.textContent).toBe('追加済み');
    expect(first?.querySelector('button')?.textContent).toBe('再追加');
    expect(element.querySelectorAll('.sample-item')[1]?.querySelector('button')?.textContent).toBe('追加');
  });
  ...
  it('読み込みに失敗したときはエラーだけを出す', () => {
    const element = renderSamplePicker([], new Set(), { onAdd: vi.fn(), onClose: vi.fn() }, '取得できません');
  ...
  it('閉じるを呼べる', () => {
    const onClose = vi.fn();
    const element = renderSamplePicker(packs, new Set(), { onAdd: vi.fn(), onClose });
